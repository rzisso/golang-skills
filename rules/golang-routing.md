# Golang Skills — Routing Rules

> **Target Activation Files:** This rule applies automatically whenever you are interacting with `**/*.go`, `**/go.mod`, `**/go.sum`, or `**/go.work`. If the current task does not involve Go development, ignore this matrix.
...
For each Go task, immediately load the **primary skill** and all applicable **secondary skills** concurrently from your global skills path. Do not wait — load them all together at the start.

| Intent | Primary Skill | Secondary Skills |
| --- | --- | --- |
